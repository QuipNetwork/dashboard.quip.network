...
describe("QuipClient v0.2", () => {
  ...
  test("getStatus narrows unknown miner type to OTHER", async () => {
    const fetchImpl = (() =>
      Promise.resolve(jsonResponse({
        success: true,
        data: {
          ss58_address: "5GPP", account_id_hex: "0x", node_id: "n", is_mining: false,
          uptime_seconds: 0, chain: { head_hash: "0x", head_number: 0 },
          miner_registered: false, miner_info: null,
    ...
    const fetchImpl = (() =>
      Promise.resolve(jsonResponse({
        ...
        data: {
          ss58_address: "5GPP", account_id_hex: "0x", node_id: "n", is_mining: false,
          uptime_seconds: 0, chain: { head_hash: "0x", head_number: 0 },
          miner_registered: false, miner_info: null, miners: [],
        },
      }))) as unknown as typeof fetch;
    const c = new QuipClient({ baseUrl: "http://x", fetchImpl });
    const r = await c.getStatus();
  ...
  test("401 raises AuthError", async () => {
    const fetchImpl = (() =>
      Promise.resolve(jsonResponse({}, 401))) as unknown as typeof fetch;
    const c = new QuipClient({ baseUrl: "http://x", fetchImpl });
  ...
  test("429 raises RateLimitError", async () => {
    const fetchImpl = (() =>
      Promise.resolve(jsonResponse({}, 429))) as unknown as typeof fetch;
    const c = new QuipClient({ baseUrl: "http://x", fetchImpl });
  ...
  test("non-OK status raises generic Error with status code", async () => {
    const fetchImpl = (() =>
      Promise.resolve(jsonResponse({}, 502))) as unknown as typeof fetch;
    const c = new QuipClient({ baseUrl: "http://x", fetchImpl });
  ...
  test("authorization header is included when token is set", async () => {
    ...
    const fetchImpl = ((_url: string, init?: { headers?: Record<string, string> }) => {
      ...
      return Promise.resolve(jsonResponse({
        success: true,
        data: {
          ss58_address: "", account_id_hex: "", node_id: "", is_mining: false,
          uptime_seconds: 0, chain: { head_hash: "", head_number: 0 },
          miner_registered: false, miner_info: null, miners: [],
        },
      }));
    }) as unknown as typeof fetch;
