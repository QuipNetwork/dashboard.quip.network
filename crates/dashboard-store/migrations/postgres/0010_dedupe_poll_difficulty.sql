...
WHERE source = 'poll'
  AND EXISTS (
    ...
    WHERE prev.observed_at_block = (
      ...
      AND prev.topology_hash IS NOT DISTINCT FROM difficulty_history.topology_hash);
...
WHERE EXISTS (
    ...
    WHERE prev.observed_at_block = (
      ...
      AND prev.topology_hash IS NOT DISTINCT FROM dashboard_poll_difficulty.topology_hash);
