...
//
//   (1) Reconcile a DRIFTED `node_descriptors`. Pre-monorepo dashboards created
//       this table under the SAME migration name (`0001_initial`) but with an
//       older column set (`payload_hash`, no `block_hash` / `extrinsic_index`).
//       Because Kysely tracks migrations by name and `0001_initial` was already
//       recorded, the rebuilt `0001_initial` (guarded with `if not exists`)
//       never reshaped the table, so the descriptor worker's INSERT failed every
//       scan ("column block_hash does not exist") and the registry froze at
//       stale rows. We drop + recreate ONLY when the legacy shape is detected
//       (no `block_hash` column): a healthy/canonical table is left untouched so
//       we never discard a DB's accumulated `first_block_timestamp` (firstSeen)
//       — which the worker reconstructs offline and cannot recover from a plain
//       head snapshot. `node_descriptors` is a latest-per-account snapshot the
//       worker rebuilds, so dropping a *drifted* table is safe.
//
...
export async function up(db: Kysely<unknown>): Promise<void> {
  // (1) Rebuild node_descriptors only if it's in the legacy (drifted) shape.
  const probe = await sql<{ has_block_hash: boolean }>`
    select exists (
      select 1 from information_schema.columns
      where table_name = 'node_descriptors' and column_name = 'block_hash'
    ) as has_block_hash
  `.execute(db);
  const canonical = probe.rows[0]?.has_block_hash === true;
  if (!canonical) {
  ...
  // (2) Per-topology analytics tags. Filtering is done in the API (in memory off
  // the recent window), so no SQL index is added until a query needs one.
