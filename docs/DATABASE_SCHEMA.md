...
| `mining_time`            | DOUBLE PRECISION | no   | Seconds of compute behind the win: miner-reported device time (µs→s, runtime-112+) or derived block spacing for pre-runtime-112/unreported wins. |
| `device_access_time_us`  | BIGINT           | yes  | Self-reported µs of device compute behind the winning proof (runtime-112+); null when absent — the normal case.                                  |
