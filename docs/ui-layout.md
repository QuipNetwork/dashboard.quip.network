...
2. QBlocks Won / Rewards Earned stat tiles (stacked, left) + `LastQBlockCard` (right) — 2-column row — details of the last qblock this node won
3. `CurrentAttemptsPanel` — table — live iteration trail for the in-flight qblock
4. `MinerStatsPanel` — stats + per-mode table — controller counters, one row per backend mode
5. `RecentMiningPanel` ("Recent Performance") — table, sortable — last 20 submissions, default QBlock# desc
6. Rank-Adjacent Miners (`NeighborsList`) — table — leaderboard window centered on this node
...
1. Last / Current QBlock Details (`LastQBlockDetailsCard` / `CurrentQBlockDetailsCard`) — detail cards (row of 2) — last/current qblock's compute, timing, energy, diversity, solutions; current card also carries live difficulty requirements
