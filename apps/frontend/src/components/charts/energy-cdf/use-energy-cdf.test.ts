...
// processor type's single top winner before the CDF sweep, so the curves show
// what the best nodes achieve rather than the network pool.
...
import { useEnergyCdf, type EnergyCdfOptions, type EnergyCdfResult } from "./use-energy-cdf";
...
function renderHook(opts: EnergyCdfOptions): { current: EnergyCdfResult } {
  const result = {
    current: { series: [], xMin: 0, xMax: 0 } as EnergyCdfResult,
  };
  function Probe(): null {
    result.current = useEnergyCdf(opts);
    return null;
  }
